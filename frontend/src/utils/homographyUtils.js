 ...
 */
function calculateNewHomography(
  originalFieldPoints,
  updatedFieldPoints,
  templatePoints,
) {
  ...
  originalFieldPoints.forEach((originalPoint, index) => {
    const updatedPoint = updatedFieldPoints.find(
      (p) => p.id === originalPoint.id,
    );
    if (updatedPoint) {
  ...
  if (srcPoints.length < 4 || dstPoints.length < 4) {
    console.error("Insufficient point correspondences:", {
      srcPoints,
      dstPoints,
    });
    throw new Error(
      "At least four point correspondences are required to compute homography.",
    );
  }
  ...
  const dstMat = window.cv.matFromArray(
    dstPoints.length,
    2,
    window.cv.CV_32F,
    [].concat(...dstPoints),
