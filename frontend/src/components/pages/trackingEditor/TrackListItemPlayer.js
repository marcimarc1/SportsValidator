...
class TrackListItemPlayer extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    // since the bBox are not deep-copied, prevProps.bBox and this.props.bBox will point to the same object
    if (this.prevPlayerId >= 0) {
      if (this.prevPlayerId !== this.props.bBox.my.player) {
        this.setState({ processingSwap: false, swapPartner: "" }, () =>
          this.closeSwap(),
        );
      }
    }
    this.prevPlayerId = this.props.bBox.my.player;
  }
    ...
    if (
      window.confirm(
        "Do you really want to delete player " +
          (this.props.bBox.my.name
            ? this.props.bBox.my.name
            : this.props.bBox.my.player) +
          "?",
      )
    ) {
  ...
  openSwap = () => {
    this.setState(
      (prevState) => ({
        swapExpanded: !prevState.swapExpanded,
        firstTime: false,
        swapSelectionVisible: false,
      }),
      () =>
  ...
  confirmSwap = () => {
    ...
    if (!this.state.swapPartner) {
      ...
      this.props.swap(this.props.bBox.my.player, this.state.swapPartner);
    }
  };
  ...
  render() {
    let classNameExpansionPostfix = this.state.swapExpanded
      ...
      : this.state.firstTime
        ? ""
        : " unexpanded";
    let classNameSwapMenuVisibility = this.state.swapSelectionVisible
      ? ""
      : " hide";

    let playersMenuEntries;
    if (this.state.swapSelectionVisible) {
      playersMenuEntries = this.props.getSwapMenuEntries().map((player) => (
    ...
    let teamMenuItems = allTeams.map((team) => (
    ...
    let name = this.props.bBox.my.name
      ? this.props.bBox.my.name
      : this.props.bBox.my.player;
    ...
    let clickItem = () => {
      ...
      if (!deselect) this.props.changeSelection("player", bBox.my.id, deselect);
    };
    ...
    return (
      ...
      >
        <div className={"TrackListItemSwapOverlay" + classNameExpansionPostfix}>
          <div
            className={
              "TrackListItemSwapOverlayContent" + classNameSwapMenuVisibility
            }
          >
            <div className={"TrackListItemSwapOverlayContentText"}>
              Swap {bBox.my.name ? bBox.my.name : "Player " + bBox.my.player}{" "}
              with{" "}
            </div>

            {/*Material UI Dropdown Select*/}
            <FormControl className={"TrackListItemSwapOverlayContentDropdown"}>
              {/*<InputLabel id="swap-dropdown-label">Player</InputLabel>*/}
              <Select
            ...
            <IconButton
              size="small"
              disabled={this.state.processingSwap}
              className={"TrackListItemSwapOverlayContentButton"}
              onClick={this.cancelSwap}
        ...
        <EditText
          className={
            "TrackListItemName" + (bBox.my.selected ? " selected" : "")
          }
        ...
        <IconButton
          ...
          onClick={this.props.blink.bind(this, bBox)}
        ...
        <IconButton
          size="small"
          className={"TrackListItemSwap"}
          onClick={this.openSwap}
