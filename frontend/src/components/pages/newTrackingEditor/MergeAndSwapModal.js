...
export default function MergeAndSwapModal({playerChosenInList, playerNameMap, mergeModalState, handleClose, swapPlayerData, mergePlayerData}) {
  ...
  const handleClick = () => {
    if(operation === 'swap')
      swapPlayerData(selectedPlayer)
    else
      mergePlayerData(selectedPlayer)
    handleClose()
  }
    ...
    <div>
      ...
      >
        ...
        <Typography sx={{mt: 2}} id="modal-modal-title" component="h2">
            {operation === 'swap' ? 'This will swap ' + playerChosenInList + ' and the chosen player below from this frame and onwards' : 'This will merge the player with frames that come later into the player with frames that come earlier to one in all frames, deleting the one that comes later.'}
        </Typography>
        <FormControl  fullWidth>
          ...
          <Button style={{marginTop: '2em'}} color='primary' variant='contained' onClick={handleClick}>Apply Changes</Button>
        </Box>
      </Modal>
    </div>
  );
