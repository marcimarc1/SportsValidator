...
class Heatmap extends Component {
  ...
  constructor(props) {
    super(props);
    this.legendTotalHeight =
      this.props.teams.length * this.legendLineHeight + this.spaceAboveLegend;
    if (this.props.positionData) {
      this.state.endFrame =
        this.props.positionData[this.props.positionData.length - 1].image_id;
  ...
  componentDidMount() {
    ...
    this.canvas = new fabric.Canvas("heatmap-canvas", {
    ...
    let lines = [
      ...
      [
        this.props.canvasWidth - strokeWidth,
        0,
        this.props.canvasWidth - strokeWidth,
      ...
      [
        0,
        this.props.canvasHeight - strokeWidth,
    ...
    lines.forEach((l) =>
      this.nonHeatmapBubbleCanvasElements.push(
        this.makeLine(l, { strokeWidth: strokeWidth }),
      ),
    );

    this.nonHeatmapBubbleCanvasElements.push(
      this.makeLine(middleLine, {
    ...
    if (this.props.positionData) {
      ...
      for (let i = 0; i < this.props.positionData.length; i = i + 1) {
        ...
        this.positionDataIndexByPlayer[player][
          this.props.positionData[i].image_id
        ] = i;
      }
      ...
      if (
        this.legendLineHeight * this.props.teams.length >
        this.legendTotalHeight
      ) {
        this.legendLineHeight =
          this.legendTotalHeight / this.props.teams.length;
      }
      ...
      for (let i = 0; i < this.props.teams.length; i++) {
        ...
        this.nonHeatmapBubbleCanvasElements.push(
          this.makeLegendEntry(
            x,
            y,
            this.props.categoryColors[teamId],
      ...
      let stateModifier = {
        maxStepSize: Math.floor(
          this.positionDataIndexByPlayer[somePlayerIndices].length / 2 + 1,
        ),
      }; // setting maxStepSize so its maximum value only results in one value/heatmapBubble per player
  ...
  drawHeatmap = () => {
    if (this.props.positionData) {
      for (let [, indexListCurrentPlayer] of Object.entries(
        this.positionDataIndexByPlayer,
      )) {
        //[, xyz] ignores first element of returned array
        for (
          let i = this.state.startFrame;
          i < this.state.endFrame;
          i = i + this.state.stepSize
        ) {
          ...
          if (indexListCurrentPlayer[i]) {
            ...
            let x =
              (annotation.bbox[0] + annotation.bbox[2] * 0.5) /
  ...
  clearHeatmap = () => {
    this.canvas.clear();
    this.nonHeatmapBubbleCanvasElements.forEach((element) =>
    ...
    return (
      <div className="Heatmap">
        <canvas
          ...
          height={this.props.canvasHeight + this.legendTotalHeight}
        ></canvas>

        <div className="HeatmapSliderContainer">
          <div className="HeatmapSlider">
            <div className="HeatmapSliderLabel">Step Size</div>
            <MaterialUISlider
          ...
          <div className="HeatmapSlider">
            <div className="HeatmapSliderLabel">Opacity</div>
            <MaterialUISlider
          ...
          <div className="HeatmapSlider">
            <div className="HeatmapSliderLabel">Size</div>
            <MaterialUISlider
