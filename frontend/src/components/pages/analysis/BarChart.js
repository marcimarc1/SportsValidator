...
class BarChart extends Component {
  ...
  componentDidMount() {
    const ctx = document
      .getElementById("Chart" + this.props.tileId)
      .getContext("2d");
    ...
    if (this.props.groupByTeams) {
      chartColors = rawData.values.map(
        (e, index) => this.colors[index % this.colors.length],
      );
      datasets = [
        {
          data: rawData.values,
      ...
      for (const t of teams) {
        ...
        let indices = booleanMask
          .map((e, index) => (e ? index : -1))
          .filter((e) => e >= 0); //generate a list of all indices of datapoints to keep

        let currentDataset = {
          label: rawData.teamNames[t],
          data: rawData.values.map((v, index) =>
            indices.includes(index) ? v : undefined,
          ), // delete values that belong to other teams
