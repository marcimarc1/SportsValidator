...
class TileTypeSelector extends Component {
  state = {
    ...
    currentNotes: "",
    visibilities: {
      // which selections should be shown depending on previous selections
      groupByTeams: false,
      teams: false,
      players: false,
      notes: false,
  ...
  render() {
    ...
    let i = 0;
    let typeMenuItems = [
      <MenuItem key={i++} value={"distance-barchart"}>
        Distance Barchart
      </MenuItem>,
      <MenuItem key={i++} value={"heatmap"}>
        Heatmap
      </MenuItem>,
      <MenuItem key={i++} value={"notes"}>
        Note
      </MenuItem>,
    ];
    ...
    if (this.props.data) {
      teamsMenuItems = Object.entries(this.props.data?.teams).map(
        ([key, value]) => {
          return (
      ...
      for (const [key, value] of Object.entries(this.props.data?.teams)) {
        if (moreThanOneTeam)
          playersMenuItems.push(
            <ListSubheader key={key + 1000}>{value.name}</ListSubheader>,
          ); // couldn't figure out a better key for ListSubheader (without there is a warning), but this is relatively safe and could only cause issues if number of players exceeds 1000
        let newPlayerMenuItems = Object.entries(value.players).map(
          ([key, value]) => {
            playerNameLookup[key] = value.name;
            return (
              <MenuItem key={key} value={key}>
                <Checkbox
                  checked={this.state.currentPlayers.indexOf(key) > -1}
                />
                <ListItemText primary={value.name} />
              </MenuItem>
            );
    ...
    return (
      <div className="TileTypeSelector">
        <div className="TileTypeSelectorFormContainer">
          ...
          <div
            className={
              "TileTypeSelectorForm" +
              (this.state.visibilities.groupByTeams ? "" : " " + hiddenClass)
            }
          >
            <FormControl fullWidth={true}>
              <FormControlLabel
                control={
                  <Switch
                    checked={this.state.groupByTeams}
                    onChange={this.handleChangeGroupByTeams}
                  />
                }
                label="Group by Teams"
                labelPlacement="end"
              />
            </FormControl>
          </div>
          ...
          <div
            className={
              "TileTypeSelectorForm" +
              (this.state.visibilities.teams ? "" : " " + hiddenClass)
            }
          >
            <FormControl fullWidth={true}>
              <InputLabel id="team-selection-multiple-checkbox-label">
                Teams
              </InputLabel>
              <Select
                ...
                renderValue={(selected) => (
                  <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                    {selected.map((value) => (
                      <Chip
                        key={value}
                        label={this.props.data.teams[value].name}
                      />
                    ))}
                  </Box>
                )}
          ...
          <div
            className={
              "TileTypeSelectorForm" +
              (this.state.visibilities.players ? "" : " " + hiddenClass)
            }
          >
            <FormControl fullWidth={true}>
              <InputLabel id="player-selection-multiple-checkbox-label">
                Players
              </InputLabel>
              <Select
          ...
          <div
            className={
              "TileTypeSelectorForm" +
              (this.state.visibilities.notes ? "" : " " + hiddenClass)
            }
          >
            <EditTextarea
