...
\documentclass[
  ...
  thesis=none,     % define the type of the thesis (student, phd, none)
...
\usepackage[%
  ...
  maxbibnames=99,
  giveninits,
  uniquename=init]{biblatex} % citation style could be adjusted here (probably. somewhere here.)
\bibliography{bibliography} % defining which .bib file to use for bibliography

% thesis metadata
\title{Interdisziplinäres Projekt: SaaS Projekt Positionsanalyse 1
}
...
\examiner{Prof.\@~Dr.\@ Martin Lames}
\supervisor{Marc Schmid}


\begin{document}
%\sloppy %with this the URLs do not extend past the rest of the text, however the layout is not as nice in general.
...
\input{chapters/Entwicklungsprozess und Dokumentation.tex}
\input{chapters/Hinweise zur Integration des Backends.tex}
\input{chapters/Vorschläge zur Weiterentwicklung.tex}
